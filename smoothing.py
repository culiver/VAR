    ...
    for idx, (img, label) in enumerate(pbar):
        ...
        with torch.inference_mode():
            ...
            while len(remaining_classes) > 0:
                ...
                # # Run inpainting.
                # smoothed_output = var.inpainting(gt_tokens, mask, cfg=cfg, top_k=1, top_p=0, label=class_labels[0], g_seed=seed)

                # smoothed_output = var.autoregressive_infer_cfg(B=1, label_B=class_labels[0], cfg=cfg, top_k=1, top_p=0, g_seed=seed, more_smooth=more_smooth)

                # Convert the output tensor to a PIL image and save.
                # Assuming output image shape is (B, 3, H, W) with values in [0, 1].
                smoothed_img_np = smoothed_output[0].permute(1, 2, 0).mul(255).clamp(0, 255).cpu().numpy().astype(np.uint8)
