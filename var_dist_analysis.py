...
def main():
    ...
    if args.plot_dist_kde and args.mode == "l2_dist":
        ...
        # Process per-sample plots for wrong conditions
        logging.info(f"Generating token distance vs. probability plots for each sample with wrong class conditions...")
        for sample_idx in sample_scale_distances_probs_d16_wrong.keys():
            # For each scale in this sample
            for scale_idx in range(len(patch_nums)):
                try:
                    # Concatenate all collected data for this scale and sample
                    all_distances_d16 = np.concatenate(sample_scale_distances_probs_d16_wrong[sample_idx][scale_idx]['distances'])
                    all_probs_d16 = np.concatenate(sample_scale_distances_probs_d16_wrong[sample_idx][scale_idx]['probs'])
                    all_distances_d30 = np.concatenate(sample_scale_distances_probs_d30_wrong[sample_idx][scale_idx]['distances'])
                    all_probs_d30 = np.concatenate(sample_scale_distances_probs_d30_wrong[sample_idx][scale_idx]['probs'])
                    ...
                    plt.ylabel('Average Probability', fontsize=12)
                    plt.yscale('log')
                    plt.title(f'Sample {sample_idx}, Scale {scale_idx} (patches: {patch_nums[scale_idx]}x{patch_nums[scale_idx]})\nAvg Prob vs Distance (Wrong Class Condition)', fontsize=14)
                    plt.legend()
                    plt.grid(True, alpha=0.3)
                    plt.tight_layout()
                    plt.savefig(osp.join(dist_kde_folder_wrong, f"{sample_idx}_{scale_idx}-layer_prob_vs_dist.png"))
                    plt.close()
                    
                except Exception as e:
                    logging.error(f"Failed to create wrong condition plot for sample {sample_idx}, scale {scale_idx}: {e}")
                    continue
                
        # Generate overall plots for wrong conditions
        logging.info(f"Generating overall token distance vs. probability plots for wrong class conditions...")
        for scale_idx in range(len(patch_nums)):
            try:
                # Concatenate all collected data for this scale across all samples
                all_distances_d16 = np.concatenate(overall_scale_distances_probs_d16_wrong[scale_idx]['distances']) if overall_scale_distances_probs_d16_wrong[scale_idx]['distances'] else np.array([])
                all_probs_d16 = np.concatenate(overall_scale_distances_probs_d16_wrong[scale_idx]['probs']) if overall_scale_distances_probs_d16_wrong[scale_idx]['probs'] else np.array([])
                all_distances_d30 = np.concatenate(overall_scale_distances_probs_d30_wrong[scale_idx]['distances']) if overall_scale_distances_probs_d30_wrong[scale_idx]['distances'] else np.array([])
                all_probs_d30 = np.concatenate(overall_scale_distances_probs_d30_wrong[scale_idx]['probs']) if overall_scale_distances_probs_d30_wrong[scale_idx]['probs'] else np.array([])
                
                if len(all_distances_d16) == 0 or len(all_distances_d30) == 0:
                    logging.warning(f"No data collected for scale {scale_idx} overall wrong condition plot")
                    continue
                ...
                if np.sum(valid_indices_d16) > 10:
                    ...
                    try:
                        ...
                        logging.warning(f"Could not fit exponential curve for D16 (wrong cond): {e}")
                ...
                if np.sum(valid_indices_d30) > 10:
                    try:
                        ...
                        logging.warning(f"Could not fit exponential curve for D30 (wrong cond): {e}")
                
                plt.xlabel('Token Distance', fontsize=14)
                plt.ylabel('Average Probability', fontsize=14)
                plt.yscale('log')
                plt.title(f'Overall Scale {scale_idx} (patches: {patch_nums[scale_idx]}x{patch_nums[scale_idx]})\nAvg Prob vs Distance Across All Samples (Wrong Class Condition)', fontsize=16)
                plt.legend(fontsize=12)
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
                plt.savefig(osp.join(dist_kde_folder_wrong, f"overall_{scale_idx}-layer_prob_vs_dist.png"), dpi=300)
                plt.close()
                
                # Create a second plot showing data density heatmap for wrong conditions
                try:
                    plt.figure(figsize=(12, 8))
                    ...
                    plt.xlabel('Token Distance', fontsize=14)
                    plt.ylabel('Log10(Probability)', fontsize=14)
                    plt.title(f'Overall Scale {scale_idx} (patches: {patch_nums[scale_idx]}x{patch_nums[scale_idx]})\nData Density Heatmap (Wrong Class Condition)', fontsize=16)
                    plt.legend(fontsize=12)
                    plt.grid(True, alpha=0.3)
                    plt.tight_layout()
                    plt.savefig(osp.join(dist_kde_folder_wrong, f"overall_{scale_idx}-layer_density_heatmap.png"), dpi=300)
                    plt.close()
                    
                except Exception as e:
                    logging.error(f"Failed to create density heatmap for scale {scale_idx} (wrong cond): {e}")
                
            except Exception as e:
                logging.error(f"Failed to create overall wrong condition plot for scale {scale_idx}: {e}")
                continue
                
        logging.info(f"Token distance vs. probability plots for wrong conditions saved to: {dist_kde_folder_wrong}")
